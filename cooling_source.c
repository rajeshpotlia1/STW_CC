...
/*changes starts here*/ 
  ...
  tau_xx = tau_xy = tau_xz = tau_yx =
      tau_yy = tau_yz = tau_zx = tau_zy =
          tau_zz = 0.0; 
/*changes ends here*/
...
//changes start here
    ...
    inv_dx3 = 1.0 / dx3[k];
    #if INCLUDE_IDIR
                dxVx = CDIFF_X1(Vx, k, j, i) * inv_dx1;
                dxVy = CDIFF_X1(Vy, k, j, i) * inv_dx1;
                dxVz = CDIFF_X1(Vz, k, j, i) * inv_dx1;
    #endif
    #if INCLUDE_JDIR
                dyVx = CDIFF_X2(Vx, k, j, i) * inv_dx2;
                dyVy = CDIFF_X2(Vy, k, j, i) * inv_dx2;
                dyVz = CDIFF_X2(Vz, k, j, i) * inv_dx2;
    #endif
    #if INCLUDE_KDIR
                dzVx = CDIFF_X3(Vx, k, j, i) * inv_dx3;
                dzVy = CDIFF_X3(Vy, k, j, i) * inv_dx3;
                dzVz = CDIFF_X3(Vz, k, j, i) * inv_dx3;
    #endif

    divV = dxVx + dyVy + dzVz;
    tau_xx = 2.0 * nu1 * dxVx - c23 * nu1 * divV;
    tau_yy = 2.0 * nu1 * dyVy - c23 * nu1 * divV;
    tau_zz = 2.0 * nu1 * dzVz - c23 * nu1 * divV;
    tau_xy = tau_yx = nu1 * (dyVx + dxVy);
    tau_xz = tau_zx = nu1 * (dzVx + dxVz);
    tau_yz = tau_zy = nu1 * (dyVz + dzVy);
    mag_tau = pow(pow(tau_xx,2.) + pow(tau_yy,2.) + pow(tau_zz,2.) + 2.*(pow(tau_xy,2.) + pow(tau_xz,2.) + pow(tau_yz,2.)),1./2.);
    t_destroy = 1./mag_tau;


    double prs_org = d->Vc[PRS][k][j][i];
    double d_intenergy = (prs_org - prs)/(g_gamma - 1.0);
    double f_m_cl = d->Vc[TRC][k][j][i];
    double T_cl = 1.e4;
    double f_v_h; 
    ...
    //if (f_m_cl == 1.0){
    d->Vc[TRC][k][j][i] = f_m_cl;
    }
    else {
    double rho_cl = prs_org*mu1*CONST_mp*UNIT_VELOCITY*UNIT_VELOCITY/(T_cl * CONST_kB);  // cold gas density   
    f_v_h = 1 - v1[RHO]*f_m_cl/rho_cl;
    double f_v_cl = 1 - f_v_h;  // cold gas volume fraction
    double rho_h = (v1[RHO] - f_v_cl*rho_cl)/f_v_h; // hot gas density
    double T_h = rho_cl*T_cl/rho_h;  // hot gas temperature
    double net_growth_radiative = fmax((d_intenergy)*2*mu1*CONST_mp*UNIT_VELOCITY*UNIT_VELOCITY/(3*v1[RHO]*CONST_kB*(T_h-T_cl)),0.0);
   // d->Vc[TRC][k][j][i] = f_m_cl + net_growth_radiative;}
 
// mixing contribution starts:
    double lambda0 = 1.1e-19;
    //double t_destroy = 6.25;//delX/V_turb; // take some constant
    double mixed_cooling_time_const_factor = pow(mu1*CONST_mp, 2.)*pow(UNIT_VELOCITY, 3.0)/(UNIT_LENGTH*UNIT_DENSITY*lambda0*(g_gamma-1.));
    double mixed_Temp = pow(T_h*1.e4, 1./3.);
    double mixed_rho_square = rho_h*rho_cl;
    ...
    printLog ("Nan found in mixed_cooling_time: f_m_cl = %12.6e, prs_org = %12.6e, mixed_Temp = %12.6e, mixed_rho_square = %12.6e, rho_h = %12.6e, rho_cl = %12.6e, T_h = %12.6e\n", f_m_cl, prs_org, mixed_Temp, mixed_rho_square, rho_h, rho_cl, T_h);
    }

    d->Vc[TRC][k][j][i] = f_m_cl + net_growth;

// mixing contribution ends:    
    }
