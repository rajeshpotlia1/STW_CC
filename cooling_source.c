...
//    T1  = prs/v1[RHO]*KELVIN*mu1;
    ...
    //if (f_m_cl>0.5) {
   // f_v_h = 1 - v1[RHO]*(f_m_cl-TOLERANCE)/rho_cl;} // hot gas volume fraction
   // else {
   // f_v_h = 1 - v1[RHO]*(f_m_cl+TOLERANCE)/rho_cl;} // hot gas volume fraction
  ...
  //  T1  = T_h;
    ...
    double cold_growth_by_mixing = pow(pow(f_m_cl,2.)*(1.-f_m_cl),3./2.)*dt/t_growth;
    double cold_dest_by_mixing = pow(pow((1.-f_m_cl),2.)*(f_m_cl),3./2.)*dt/t_destroy;
